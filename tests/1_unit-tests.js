...
suite('UnitTests', () => {
  ...
  suite('Function ____()', () => {
    test('Valid "1-9" characters', (done) => {
      ...
      input.forEach(i => assert.equal(Solver.isInputNum(i), true));
      done();
    });
    ...
    test('Invalid characters (anything other than "1-9") are not accepted', (done) => {
      ...
      input.forEach(i => assert.equal(Solver.isInputNum(i), false));
      done();
    });
  });
  ...
  suite('Function ____()', () => {
    test('Parses a valid puzzle string into an object', done => {
      ...
      assert.isObject(Solver.parseToObj(input));
      done();
    });
    ...
    test('Shows an error for puzzles that are not 81 numbers long', done => {
      ...
      Solver.isValidInput(shortStr)
      assert.equal(errorDiv.innerText, errorMsg)
      Solver.isValidInput(longStr)
      assert.equal(errorDiv.innerText, errorMsg)
      done();
    });
  });
